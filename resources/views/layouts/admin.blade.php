        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <nav class="relative flex-1 px-3 py-6 space-y-1">
                @foreach([
                    ...
                    ['route' => 'admin.invoices.index', 'pattern' => 'admin.invoices.*|admin.orders.invoice.*|admin.billing.*', 'label' => 'Billing', 'permission' => 'billing.view', 'icon' => 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z', 'children' => [
                        ['route' => 'admin.invoices.index', 'pattern' => 'admin.invoices.*|admin.orders.invoice.*', 'label' => 'Invoices', 'permission' => 'billing.view'],
                        ['route' => 'admin.billing.settings', 'pattern' => 'admin.billing.*', 'label' => 'Billing Settings', 'permission' => 'billing.settings'],
                    ]],
                    ['route' => 'admin.catalog.index', 'pattern' => 'admin.catalog.*', 'label' => 'Catalog', 'permission' => 'catalog.view', 'icon' => 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z'],
                    ['route' => 'admin.raw-materials.index', 'pattern' => 'admin.raw-materials.*', 'label' => 'Raw Materials', 'permission' => 'raw-materials.view', 'icon' => 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4'],
                    ['route' => 'admin.reports.index', 'pattern' => 'admin.reports.*', 'label' => 'Reports', 'permission' => 'reports.view', 'icon' => 'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z'],
                    ...
                    @can('permission', $item['permission'])
                        @if(! empty($item['children']))
                            @php
                                $patterns = array_map('trim', explode('|', $item['pattern']));
                                $groupActive = request()->routeIs(...$patterns);
                            @endphp
                            <div x-data="{ open: {{ $groupActive ? 'true' : 'false' }} }">
                                <button type="button" @click="open = !open"
                                    class="jewel-sidebar-link w-full justify-between {{ $groupActive ? 'jewel-sidebar-link-active' : 'jewel-sidebar-link-inactive' }}">
                                ...
                                <div x-show="open"
                                    ...
                                    class="mt-1 ml-4 pl-3 border-l border-white/10 space-y-0.5">
                                    @foreach($item['children'] as $child)
                                        @can('permission', $child['permission'])
                                            <a href="{{ route($child['route']) }}"
                                                class="jewel-sidebar-sublink {{ request()->routeIs($child['pattern']) ? 'jewel-sidebar-sublink-active' : 'jewel-sidebar-sublink-inactive' }}">
                                                {{ $child['label'] }}
                                            </a>
                                        @endcan
                                    @endforeach
                    ...
                    <x-alert type="warning" class="mb-6">
                        ...
                        <p class="mt-1">
                            @if(($deliveryOverdueCount ?? 0) > 0)
                                <span class="font-medium">{{ $deliveryOverdueCount }} overdue deliveries</span>
                                @if(($deliveryDueSoonCount ?? 0) > 0) · @endif
                            @endif
                            @if(($deliveryDueSoonCount ?? 0) > 0)
                                <span class="font-medium">{{ $deliveryDueSoonCount }} due soon</span>
                            @endif
                        </p>
                    </x-alert>
                @endif
